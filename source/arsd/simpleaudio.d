...
final class AudioPcmOutThreadImplementation : Thread
{
	...
	@scriptable void beep(int freq = 900, int dur = 150, int volume = DEFAULT_VOLUME)
	{
		Sample s;
	...
	///
	@scriptable void noise(int dur = 150, int volume = DEFAULT_VOLUME)
	{
		Sample s;
		s.operation = 1; // noise
		s.frequency = 0;
	...
	///
	@scriptable void boop(float attack = 8, int freqBase = 500, int dur = 150,
			int volume = DEFAULT_VOLUME)
	{
		...
		s.f = delegate short(int x) {
			auto currentFrequency = cast(float) freqBase / (
					1 + cast(float) x / (cast(float) SampleRate / attack));
			import std.math;
	...
	///
	@scriptable void blip(float attack = 6, int freqBase = 800, int dur = 150,
			int volume = DEFAULT_VOLUME)
	{
		Sample s;
		s.operation = 5; // custom
	...
	version (none) void custom(int dur = 150, int volume = DEFAULT_VOLUME)
	{
		Sample s;
		s.operation = 5; // custom
		s.volume = volume;
	...
	void playOgg()(string filename, bool loop = false)
	{
		...
		addChannel(delegate bool(short[] buffer) {
...
struct AudioInput
{
	...
	this(int card, int SampleRate = 44100, int channels = 2)
	{
		...
		{
			event = CreateEvent(null, false  /* manual reset */ , false  /* initially triggered */ ,
					null);
			...
			format.cbSize = 0;
			if (auto err = waveInOpen(&handle, WAVE_MAPPER, &format,
					cast(DWORD_PTR) event, cast(DWORD_PTR)&this, CALLBACK_EVENT))
	...
	version (ALSA) short[] read(short[] buffer)
	{
		...
		else version (WinMM)
		{
			...
			while (recording)
			{
				...
				foreach (ref header; headers)
				{
					...
					receiveData(
							(cast(short*) header.lpData)[0 .. header.dwBytesRecorded / short.sizeof]);
					if (!recording)
						break;
					header.dwUser = 1; // mark that the driver is using it
					if (auto err = waveInAddBuffer(handle, &header, header.sizeof))
			...
			*/
			...
			still_in_use: foreach (idx, header; headers)
				if (!(header.dwFlags & WHDR_DONE))
				{
					Sleep(1);
					goto still_in_use;
...
struct AudioOutput
{
	...
	this(int card, int SampleRate = 44100, int channels = 2)
	{
		...
		else version (WinMM)
		{
			...
			format.wBitsPerSample = 16;
			format.cbSize = 0;
			if (auto err = waveOutOpen(&handle, WAVE_MAPPER, &format,
					cast(DWORD_PTR)&mmCallback, cast(DWORD_PTR)&this, CALLBACK_FUNCTION))
				...
				if (playing)
				{
					...
					while (data.length)
					{
						...
						if (written < 0)
						{
							...
							if (written < 0)
								throw new AlsaException("pcm write", cast(int) written);
						}
	...
	version (WinMM)
	{
		extern (Windows) static void mmCallback(HWAVEOUT handle, UINT msg,
				void* userData, WAVEHDR* header, DWORD_PTR param2)
		{
			...
			if (msg == WOM_DONE)
			{
				...
				{
					ao.fillData(
							(cast(short*) header.lpData)[0 .. header.dwBufferLength / short.sizeof]);
				}
				header.dwUser = 1;
			}
		}
...
B0 40 00 # sustain pedal off
	...
	this(int card)
	{
		...
		{
			if (auto err = midiInOpen(&handle, 0, cast(DWORD_PTR)&mmCallback,
					cast(DWORD_PTR)&this, CALLBACK_FUNCTION))
	...
	version (WinMM) private void delegate(uint timestamp, ubyte b1, ubyte b2, ubyte b3) dg;

	version (WinMM) extern (Windows) static void mmCallback(HMIDIIN handle,
			UINT msg, DWORD_PTR user, DWORD_PTR param1, DWORD_PTR param2)
	{
		MidiInput* mi = cast(MidiInput*) user;
		if (msg == MIM_DATA)
		{
			mi.dg(cast(uint) param2, param1 & 0xff, (param1 >> 8) & 0xff, (param1 >> 16) & 0xff);
		}
	}
...
version (Posix)
{
	...
	private extern (C) void interruptSignalHandler(int sigNumber) nothrow
	{
		interrupted = true;
	}
}
...
struct AudioMixer
{
	...
	this(int cardId)
	{
		...
		version (ALSA)
		{
			...
			if (auto err = snd_mixer_selem_get_playback_volume_range(selem,
					&minVolume, &maxVolume))
	...
	version (ALSA) version (with_eventloop)
	{
		static struct MixerEvent
		{
		}

		nothrow @nogc extern (C) static int alsaCallback(snd_mixer_elem_t*, uint)
		{
			import arsd.eventloop;
...
/// Controller.
ubyte[] midiNoteController(ref ubyte[] where, ubyte channel,
		byte controllerNumber, byte controllerValue)
{
...
version (ALSA) // Opens the PCM device with default settings: stereo, 16 bit, 44.1 kHz, interleaved R/W.
snd_pcm_t* openAlsaPcm(snd_pcm_stream_t direction, int SampleRate, int channels)
{
	...
	if (auto err = snd_pcm_hw_params_any(handle, hwParams)) // can actually survive a failure here, we will just move forward
	{
	} // throw new AlsaException("params init", err);

	if (auto err = snd_pcm_hw_params_set_access(handle, hwParams,
			snd_pcm_access_t.SND_PCM_ACCESS_RW_INTERLEAVED))
		throw new AlsaException("params access", err);

	if (auto err = snd_pcm_hw_params_set_format(handle, hwParams,
...
version (ALSA) class AlsaException : AudioException
{
	this(string message, int error, string file = __FILE__,
...
version (WinMM) class WinMMException : AudioException
{
	this(string message, int error, string file = __FILE__,
			size_t line = __LINE__, Throwable next = null)
	{
...
@nogc nothrow:
	...
	enum snd_pcm_format
	{
		...
		SND_PCM_FORMAT_LAST = SND_PCM_FORMAT_DSD_U16_LE,// I snipped a bunch of endian-specific ones!
	}
	...
	alias snd_lib_error_handler_t = void function(const(char)* file, int line,
			const(char)* function_, int err, const(char)* fmt, ...);
	int snd_lib_error_set_handler(snd_lib_error_handler_t handler);

	import core.stdc.stdarg;

	private void alsa_message_silencer(const(char)* file, int line,
			const(char)* function_, int err, const(char)* fmt, ...)
	{
	}
